function pith_line=shadedErrorBarImproved(x,y,k,errBar,varargin)
...
H = makePlot(x,y,k,errBar,lineProps,transparent,patchSaturation);
...
function H = makePlot(x,y,k,errBar,lineProps,transparent,patchSaturation)
    ...
    if isdatetime(x) && strcmp(hostName,'MATLAB')
      H.patch=patch(datenum(xP),yP+0.2,1);
    else
      H.patch=patch(xP,yP+0.2,1);
    end
