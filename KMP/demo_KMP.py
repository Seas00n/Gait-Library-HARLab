...
plt.plot(train_data[0, :, 0].T, train_data[0, :, 1].T, c='k', alpha=0.1, label="Training Data")
...
Ref_Old = plt.plot(phase_ref, Data_ref[:, 0], c='r', lw=2, alpha=0.5, label="GMM-GMR Result")
plt.plot(phase_ref, Data_ref[:, 1], c='r', lw=2, alpha=0.5)
plt.xlabel("Phase")
plt.ylabel("Fz/M(m·s-2)")
plt.title("GMM-GMR Result")
plt.legend()
plt.show()
...
test_plot = plt.plot(test_phase, test_data[:, 0], 'b--', lw=2, label="Test Data")
plt.plot(test_phase, test_data[:, 1], 'b--', lw=2)
...
plt.scatter(via_phase, via_point[:, 0], linewidths=5, label="Feature Point")
plt.scatter(via_phase, via_point[:, 1], linewidths=5)
plt.xlabel("Phase")
plt.ylabel("Fz/M(m·s-2)")
plt.title("Test Data and Fea Point")
plt.legend()
plt.show()
...
plt.plot(test_phase, test_data[:, 0], 'b--', lw=2,alpha=0.5,label="Test Data")
plt.plot(test_phase, test_data[:, 1], 'b--', lw=2,alpha=0.5)
plt.scatter(via_phase, via_point[:, 0], linewidths=5, label="Feature Point")
plt.scatter(via_phase, via_point[:, 1], linewidths=5)
test_data_with_via_point_plot = plt.plot(newPhase, newData[:, 0], 'r', alpha=0.2, lw=4, label="Ref Data with Fea Point")
plt.plot(newPhase, newData[:, 1], 'r', lw=4, alpha=0.2)
...
finalTraj = kmp_interp_with_phase(phase_new=finalPhase,
                                  ...
                                  kh=kh,
                                  Kinv=Kinv)
kmp_plot = plt.plot(finalPhase, finalTraj[:, 0], 'm', label="KMP Traj",lw=5)
plt.plot(finalPhase, finalTraj[:, 1], 'm',lw=5)
plt.xlabel("Phase")
plt.ylabel("Fz/M(m·s-2)")
plt.title("KMP Result")
plt.legend()
plt.show()
